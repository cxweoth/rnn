...
class tensor:
    ...
    # =====================================================
    # INDEXING / SLICING (keeps graph!)
    # =====================================================
    def __getitem__(self, idx):
        out = self.data[idx]
        return tensor(
            out,
            grad_fn=SliceBackward(self, idx, self.data.shape),
...
class AddBackward:
    ...
    def backward(self, grad_output):
        ...
        return [(self.left, grad_left), (self.right, grad_right)]
...
class SigmoidBackward:
    ...
    def backward(self, grad_output):
        grad = self.out_data * (1.0 - self.out_data) * grad_output
        return [(self.parent, grad)]
...
class StackBackward:
    ...
    def backward(self, grad_output):
        ...
        for i, parent in enumerate(self._parents):
            grads.append((parent, grad_output[i]))
        return grads


class SliceBackward:
    def __init__(self, parent, idx, original_shape):
        self.parent = parent
        self.idx = idx
        self.original_shape = original_shape
    ...
    def backward(self, grad_output):
        ...
        # Works for int/slice/tuple slicing. For repeated fancy-index entries,
        # this is not perfect, but fine for typical time/batch slicing.
        grad[self.idx] += grad_output
        return [(self.parent, grad)]